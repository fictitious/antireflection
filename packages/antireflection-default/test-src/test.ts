...
suite('B', function() {
    test('a', function() {
        ...
        assert.match(r1.formatDiagnostic(r1.diagnostics[0]), /Type '{ defaultValue: string; .* is not assignable to type '\(T<"object"> & { p: \(\) => Properties; _p: Properties\[\]; } & { defaultValue\?: number;/);
        ...
        assert.match(r2.formatDiagnostic(r2.diagnostics[0]), /Type '{ defaultValue: string; .* is not assignable to type '\(T<"object"> & { p: \(\) => Properties; _p: Properties\[\]; } & { defaultValue\?: number;/);
